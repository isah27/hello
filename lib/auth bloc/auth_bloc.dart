...
class AuthBloc extends Cubit<SwiftAccessStates> {
  ...
  formValidate({required List<TextEditingController> controllers}) {
    emit(FormValidatingState());
    if (controllers.any((controller) => controller.text == "")) {
      formValidated = false;
    } else {
      formValidated = true;
    }
  ...
  loginFormValidation() {
    formValidate(controllers: [emailController, passwordController]);
  }
